...
*Setup instructions for a gitian build of BHash using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BHash
...
Clone the git repositories for bhash and gitian and then checkout the bhash version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bhashcoin/bhash.git
cd bhash
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bhash repository
...
To build BHash (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bhash repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bhash/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/bhashcoin/bhash
...
For example:
```bash
URL=https://github.com/crowning-/bhash.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bhash=${COMMIT} --url bhash=${URL} ../bhash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bhash=${COMMIT} --url bhash=${URL} ../bhash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bhash=${COMMIT} --url bhash=${URL} ../bhash/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bhash-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bhash-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bhash-build.assert
```
...
[bhash/gitian.sigs](https://github.com/bhashcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
