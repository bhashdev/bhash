...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::HASH)
    {
    }
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeBHashAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBHashAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeBHashAmount = strAnonymizeBHashAmount.remove(strAnonymizeBHashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBHashAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeBHashAmount * COIN) nMaxToAnonymize = nAnonymizeBHashAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeBHashAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeBHashAmount));
        strAnonymizeBHashAmount = strAnonymizeBHashAmount.remove(strAnonymizeBHashAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBHashAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
