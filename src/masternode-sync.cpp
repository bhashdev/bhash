...
{
    //std::cout << "CMasternodeSync::AddedMasternodeList: " << hash.ToString() << std::endl;
...
{
    //std::cout << "CMasternodeSync::AddedMasternodeWinner: " << hash.ToString() << std::endl;
		...
		LogPrint("masternode", "CMasternodeSync::Process() - lastMasternodeList %lld (GetTime() - MASTERNODE_SYNC_TIMEOUT) %lld\n", lastMasternodeList, GetTime() - MASTERNODE_SYNC_TIMEOUT);
                ...
                if (lastMasternodeList == 0 &&
                    ...
                    if (IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) { 
                        LogPrintf("CMasternodeSync::Process - ERROR - Sync has failed, will retry later (1)\n");
            ...
            if (RequestedMasternodeAssets == MASTERNODE_SYNC_MNW) {
                ...
                if (lastMasternodeWinner == 0 &&
                    (RequestedMasternodeAttempt >= MASTERNODE_SYNC_THRESHOLD * 3 || GetTime() - nAssetSyncStarted > MASTERNODE_SYNC_TIMEOUT * 5)) {
                    if (IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) {
                        LogPrintf("CMasternodeSync::Process - ERROR - Sync has failed, will retry later (2)\n");
                        RequestedMasternodeAssets = MASTERNODE_SYNC_FAILED;
                        RequestedMasternodeAttempt = 0;
                        lastFailure = GetTime();
                        nCountFailures++;
