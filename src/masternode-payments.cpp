...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    // If the block is below the slow start PoW 100% time frame
    // then ignore filling in the masternode payee.
    CAmount blockValue = GetBlockValue(pindexPrev->nHeight);
    if (pindexPrev->nHeight + 1 <= 3360) {
    ...
    if (!masternodePayments.GetBlockPayee(pindexPrev->nHeight + 1, payee)) {
        ...
        //std::cout << "No MN " << winningNode->addr.ToString() << " " << pindexPrev->nHeight + 1 << std::endl;
        if (winningNode) {
...
{ 
    //std::cout << "CMasternodePayments::GetBlockPayee:" << nBlockHeight << " " << payee.ToString() << std::endl; 
