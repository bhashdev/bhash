...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    //std::cout << "vout[0].nValue: " << txNew.vout[0].nValue << std::endl;
...

    {
        ...
        if (!fProofOfStake) {
            ...
            //std::cout << "vout size: " << txNew.vout.size() << std::endl;
            //std::cout << "vout[0].nValue: " << txNew.vout[0].nValue << std::endl;

            //Make payee
            if (txNew.vout.size() > 1) {
                //LogPrintf("FillBlockPayee vout size: %d\n", txNew.vout.size());
        ...
        if (!fProofOfStake) {
            pblock->vtx[0] = txNew;
            //std::cout << "nFees: " << nFees << std::endl;
            pblocktemplate->vTxFees[0] = -nFees;
            //std::cout << "vTxFees[0]: " << pblocktemplate->vTxFees[0] << std::endl;
        }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BHashMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BHashMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BHashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bhash-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        // [hetus] TODO: Look for address with miner label.
        ...
        LogPrintf("Running BHashMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
