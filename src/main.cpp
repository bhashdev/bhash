...
#if defined(NDEBUG)
#error "BHash cannot be compiled without assertions."
#endif
...
{
    int64_t nSubsidy = 8 * COIN; // Forever!!!

    if (nHeight == 0) {
	    nSubsidy = 300000 * COIN; // "premine" 300k
    } else if (nHeight >= 1 && nHeight <= 3360) {
        nSubsidy = 0;
    } else if (nHeight >= 3361 && nHeight <= 4800) {
        nSubsidy = 10 * COIN;
    } else if (nHeight >= 4801 && nHeight <= 172800) {
        nSubsidy = 20 * COIN;
    } else if (nHeight >= 172801 && nHeight <= 345600) {
        nSubsidy = 16 * COIN;
    } else if (nHeight >= 345601 && nHeight <= 518400) {
        nSubsidy = 12 * COIN;
    }
...
int ActiveProtocol()
{
    // If 15 is active then return the spork value.
    if (IsSporkActive(SPORK_15_NEW_PROTOCOL_ENFORCEMENT))
        return GetSporkValue(SPORK_15_NEW_PROTOCOL_ENFORCEMENT);

    // Return predetermined cutoff protocol when active.
    if (IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
        return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // Return the current min. valid protocol.
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        if (!fRet) {
            //std::string vs(vRecv.begin(), vRecv.end());
            //std::cout << strCommand << " " << vs << " " << msg.nTime << std::endl;
