...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnb") { //Masternode Broadcast
        ...
        if (!obfuScationSigner.IsVinAssociatedWithPubkey(mnb.vin, mnb.pubKeyCollateralAddress)) {
            ...
            LogPrintf("%s %s", mnb.vin.ToString(), mnb.pubKeyCollateralAddress.GetHash().ToString());
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (addr.GetPort() != 17652) return;
        } else if (addr.GetPort() == 17652)
            return;
	...
	LogPrint("masternode", "dsee - Got NEW OLD Masternode entry %s\n", vin.prevout.hash.ToString());
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 2000 HASH tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
		...
		// block for 2000 HASH tx -> 1 confirmation
